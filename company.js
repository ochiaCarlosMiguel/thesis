...
// Function to enable editing of company names
document.querySelectorAll('.edit-icon').forEach(button => {
    button.addEventListener('click', function () {
        const brandText = this.previousElementSibling;
        const newBrandName = prompt('Edit Company Name:', brandText.textContent);
        if (newBrandName) {
...
// Function to delete company items
document.querySelectorAll('.deleteBtn').forEach(button => {
    button.addEventListener('click', function () {
        const listItem = this.closest('.companyItem');
        listItem.remove();
    });
});
