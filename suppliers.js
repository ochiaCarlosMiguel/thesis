...
// Function to enable editing of supplier names
document.querySelectorAll('.edit-icon').forEach(button => {
    button.addEventListener('click', function () {
        const brandText = this.previousElementSibling;
        const newBrandName = prompt('Edit Supplier Name:', brandText.textContent);
        if (newBrandName) {
...
// Function to delete supplier items
document.querySelectorAll('.deleteBtn').forEach(button => {
    button.addEventListener('click', function () {
        const listItem = this.closest('.supplierItem');
        listItem.remove();
    });
});
